...
new Handle:cknife;
bool clientMass[MAXPLAYERS] = false;
...
public OnPluginStart()
{
	...
	HookEvent("weapon_fire", proverka, EventHookMode_Pre);
}

public void proverka(Event event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	char weaponBuff[32];
	GetEventString(event, "weapon", weaponBuff, sizeof(weaponBuff));
	if (StrContains(weaponBuff, "flashbang", false) || StrContains(weaponBuff, "smokegrenade", false) || StrContains(weaponBuff, "hegrenade", false))
	{
...
public Action:blocktimer(Handle:timer, any:client)
{
	clientMass[client] = false;
}
...
public Action:Drop(client, const String:command[], argc)
{
	...
	{
		if(StrEqual(name, "weapon_flashbang", false) && (clientMass[client] != true))
		{
		...
		else if(StrEqual(name, "weapon_hegrenade", false) && (clientMass[client] != true))
		{
		...
		else if(StrEqual(name, "weapon_smokegrenade", false) && (clientMass[client] != true))
		{
